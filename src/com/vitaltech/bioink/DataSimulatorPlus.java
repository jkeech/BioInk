	...
	public void run(){
		...
		try {
			while(run){
				...
				syncUsers(BiometricType.HRV, 0.1f * dataProcessor.maxHRV);
				Thread.sleep(2 * wait);
				if(!run)break;
				...
				syncUsers(BiometricType.RR, 0.5f * dataProcessor.maxHRV);
				Thread.sleep(2 * wait);
				if(!run)break;
...
//				Thread.sleep(2 * wait);
	...
	private void threeTypes(String user, float value){
		...
		dataProcessor.push(user, BiometricType.RR, value * dataProcessor.maxHRV);
	}
